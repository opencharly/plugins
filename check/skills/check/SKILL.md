...
| **Harness project configuration** — `AGENTS.md`, `.codex/**`, repo-native `.agents/**`, or an executable that provisions or validates those surfaces | Parse the changed configuration; run the project profile and static-validator checks; verify exact gitlink and linked-worktree provenance | Directly execute each changed repository validator/provisioning entrypoint. A Git-provisioning change also proves its alternate/reference behavior against a disposable linked-worktree spike. The fresh validator then requests and executes every boundary command its independently derived R10 needs; a denied command is `BLOCKED`. Repository validation proves declared configuration and executable repository behavior — never host-owned effective sandbox/approval settings or platform role discovery. When the host supplies an attestation bound to the candidate tree and configuration, record it as supplemental runtime-admission evidence; its absence cannot be replaced by a restart, wrapper, clone, alternate home, or temporary workspace. The fresh validator independently decides whether the changed final tree additionally requires a Charly R10 bed and runs it when required. | `fully tested and validated` | An unrelated VM/container roster or an agent-created session-restart ritual that cannot exercise repository behavior or permissions |
