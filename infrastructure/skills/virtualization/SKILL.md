...
`charly/vm_backend_lifecycle.go` already hardcodes `qemu:///session` as the charly default.
...
- `/charly-vm:vm` — VM lifecycle (build, create, start, stop, ssh, console, destroy); defaults to `qemu:///session` at charly/vm_backend_lifecycle.go
