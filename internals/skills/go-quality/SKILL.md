...
2. **AST-aware rewrite over regex, wherever available.** `gofmt -r 'Symbol -> pkg.Symbol'` operates on the parsed AST and inherently cannot touch selector fields, method names, composite-literal keys, or local identifiers — they are different AST node kinds from a bare type reference. Prefer it for any symbol with a non-trivial collision surface; a maximally-generic name (class 6) is the highest-risk case. Fall back to per-file hand edits — never regex — for anything `gofmt -r` cannot express (e.g. qualify-only-in-non-test scoping); add the import separately (`gofmt -r` does not manage imports).
3. **The compiler is the final gate, never a formality.** `go build` and `go vet` catch every remaining defect precisely — an invalid field name, an ambiguous identifier, an unused import — before a single test runs. Never ship a repoint the compiler has not actually built against the FULL final diff; a green build/vet on the complete, final tree is the acceptance bar, not a smoke check.
