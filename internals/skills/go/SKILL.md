...
| ~~`check_runner_cmd.go`~~ | DELETED (K5-U2/3, the check-config seam's death). `scorePodTargetEntry`'s successor is `host_build_pod_disposable.go`'s thin "pod-disposable" host seam, which `candy/plugin-check/checkproject.go`'s `podDisposable` calls (the ONE check-project fact `spec.ResolvedProject` cannot carry); the check-run management Cmds and the orchestrator preflight live in the `command:check` plugin `candy/plugin-check`. |
...
| `host_build_check_bed.go` | The narrow op-discriminated "check-bed" host HELPER seam (K5-U1: the SESSION itself — locks/lease/env/BedDescriptor — moved INTO the compiled-in plugin, `candy/plugin-check/bed_session.go`, which shares the host's OS process). What remains: `members-up`/`members-down`/`wait-ready` (saveDeployState + libvirt + SSHExecutor/podman polls, shared with `bundle_members.go`), `persist-overrides` (wraps `persistBedDeployOverrides`, same sharing), `gpu-prereq` (wraps `bedGPUPrereqMissing`, needs the project's resolved resource vocabulary), `libvirt-start` (a best-effort systemd kick shared with 2 other core callers), `lease-acquire`/`lease-release` (wrap `acquireResourceForClaimant`, shared with `substrate_lifecycle_grpc.go`), and `repo-override-pair` (a stateless git-identity read the plugin's session cannot self-serve — returns a pair STRING only; the plugin does the actual env set/restore itself). |
| ~~`host_build_check_config.go`~~ | DELETED (K5-U2/3, the check-config seam's death). The check-project reads it served (bed-vs-iterate classify, sandbox class, resolved iterate config, include-expanded plan, kind:agent catalog) are now derived PLUGIN-SIDE off `spec.ResolvedProject` (`candy/plugin-check/checkproject.go`'s `resolveCheckProjection`); the one fact the envelope cannot carry (pod-target disposability) rides the thin `host_build_pod_disposable.go` seam. |
| `check_bed_run.go` | The KEPT bed-seam helpers genuinely SHARED with the deploy path (`bundle_members.go`): `persistBedDeployOverrides`, `bedExternalInPlace` (persistBedDeployOverrides's own in-place skip check), `deployNestedLocalChildren`, `waitForVmSshReady`/`waitForContainerReady`. The per-deploy domain locks (`bedVmDomains`/`acquireVmDomainLock`, #33) and the acceptance-depth resolver (`bedCheckLevel`) moved INTO the plugin (K5-U1 — check-bed-exclusive, zero other core caller): `bedVmDomains` is now pure logic in `candy/plugin-check/bed_session.go` locking directly via `kit.AcquireFileLock`; `bedCheckLevel`'s job is now a direct `rp.Boxes[image].CheckLevel` read off the resolved-project envelope, no host call at all. The bed-run ORCHESTRATION + its OWN session state live in `candy/plugin-check/bed_run.go` + `bed_session.go` (driven over this seam + `HostBuild("cli")`). |
