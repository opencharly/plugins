...
| `generate.go:emitBuilderStages` / `emitBuilderArtifacts` / `resolveDetectionBuilder` | The DETECTION-builder BUILDER leg (C10): for each candy a builder DETECTS, connects the plugin (`ensureBuildersConnected`) + `Invoke(OpResolve)` via the shared `resolveBuilderStage` → `spec.BuilderResolveReply` (`Stage` pre-main-FROM, `CopyArtifacts`+`CopyBinary` post-main-FROM; cargo's `InlineFragment` splices in `WriteCandySteps` (deploykit)). Renders via the plugins' `kit.BuilderResolve`, NOT an in-core vocabulary. Detection stays host-side (`candyNeedsBuilder`) |
