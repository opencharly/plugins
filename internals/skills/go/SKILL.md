...
- **The alias surface (v2 migration INVENTORY — every `charly/*_aliases.go` exits at K3/K4/K5 as its call sites move into plugins; the compile-time parity gate below is a TRANSITIONAL convenience, NOT a permanent design virtue — see CLAUDE.md "Core is a PLUGIN HOST" + the ZERO-ALIASES standing rule).** Most authored param types are referenced FULLY-QUALIFIED as `spec.X` directly (`spec.BoxConfig`, `spec.Op`, `spec.CandyYAML`, `spec.ServiceEntry`, …) — package main defines NO bare alias for these at all, and the hand struct DEFINITIONS were deleted; this is the state AFTER the wave-2 `vmshared`→`spec` envelope-route repoint (23 symbols, ~200 files). A SMALL REMAINING SUBSET — the VM/cloud-init/android-specific types (`VmSpec`, `VmSource`, `AliasYAML`, `BuilderDef`, `SnapshotEntry`, …, ~34 names) — still route through a genuine bare package-main alias: `type X = vmshared.X` defined in the `vmshared` package's OWN `sdk/vmshared/spec_aliases.go` and re-exported into package main by `charly/vmshared_aliases.go` (kit-engine types similarly via `charly/kit_aliases.go`, also a small subset — see the `checkspec.go` row above for exactly which check-variable-grammar symbols are and are not aliased there; NOT every symbol a file's doc comment names is actually aliased, so grep-confirm before citing one). Because every reference — a direct `spec.X` or a `charly/vmshared_aliases.go` alias — compiles against the real spec/vmshared types, **the Go compiler IS the field-parity check**: a renamed or removed spec field (or wire-key/type change) fails the build at this surface. Name collisions where the package-main type is a different concept stay hand-written (`CalVer`, `CandyRef`, the runtime `Candy` — the authored-config type is `spec.CandyYAML`, itself an intra-`spec`-package alias to `Candy` at `sdk/spec/charly_names.go`, not a charly-core alias).
...
5. **The drift gates (keep them all green).** There is no `spec_parity_test.go`;
   ...
   charly-core alias at all); a small remaining subset of VM/cloud-init-specific
   types are genuinely aliased (`type X = vmshared.X`, defined in the `vmshared`
   package's own `sdk/vmshared/spec_aliases.go`, re-exported via
   `charly/vmshared_aliases.go` — see "The alias surface" above for the full
   detail) — either way, a hand-referenced field that no longer has a
