...
| `scaffold_cmds.go` | Kong command structs for the MCP-first BOX authoring surface (the `box new project/box/candy` scaffold group moved to the compiled-in `command:box` plugin, candy/plugin-box, in P15): `BoxSetCmd` (calls `kit.SetByDotPath`), `BoxAddCandyCmd`, `BoxRmCandyCmd`, `BoxFetchCmd`, `BoxRefreshCmd`, `BoxWriteCmd`, `BoxCatCmd`. Houses `resolveProjectFile()` — the path-traversal guard for `box write` / `box cat`. (The `candy` authoring command is a compiled-in `command:candy` plugin — `candy/plugin-candy/command.go` — not a core Kong struct.) |
