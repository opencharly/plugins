...
**Generation coverage — the wire-type mandate + the language-forced exceptions.**
...
**hand-writing a wire struct is FORBIDDEN.** The ONLY hand-written generation code
is the handful of shapes Go's type system GENUINELY cannot express (each `@go(-)`'d
+ kept in lockstep with its def) — a LANGUAGE limit, never a discretionary choice;
a new wire type is DESIGNED to avoid them so it always generates:

- `sdk/spec/union_types.go` — faithful union/shorthand types for CUE disjunctions (`gengotypes` degrades a disjunction to `any` — Go has no sum type); the matching CUE def is `@go(-)`'d. A wire type that would carry a union is a DISCRIMINATED struct instead (a `Kind`/discriminator field + per-variant optional fields — the `deploy_wire` `ReverseOp` tagged-union pattern, which generates) and REFERENCES a disjunction type by name; it never redefines one.
- `sdk/spec/hand_state_types.go` — open-tailed struct+map shapes `gengotypes` cannot express faithfully (`PortSpec`, the open-tailed `VmDeployState`), mirrored against `@go(-)`'d defs. A new wire type uses an EXPLICIT map field instead of an open tail so it generates.
...
- `sdk/proto/plugin.proto` — the RPC message schema is hand-maintained BY DESIGN (payloads ride as JSON of the `spec` types, avoiding a parallel proto type system); the Go stubs are generated from it. Not a spec wire STRUCT.
...
| `scaffold_cmds.go` | Kong command structs for the MCP-first BOX authoring surface: `NewProjectCmd`, `NewBoxCmd`, `BoxSetCmd` (calls `kit.SetByDotPath`), `BoxAddCandyCmd`, `BoxRmCandyCmd`, `BoxFetchCmd`, `BoxRefreshCmd`, `BoxWriteCmd`, `BoxCatCmd`. Houses `resolveProjectFile()` — the path-traversal guard for `box write` / `box cat`. (The `candy` authoring command is a compiled-in `command:candy` plugin — `candy/plugin-candy/command.go` — not a core Kong struct.) |
...
| `cdp_preresolve.go` | Retains only the `devToolsTab` /json decode struct the host's status-probe path still uses; the cdp endpoint resolution is the `cc.ResolveEndpoint` reverse-leg. The `cdp:` verb (open/list/close/text/html/url/screenshot/click/type/eval/wait/coords/raw + spa-*) and its CDP WebSocket client live entirely out-of-process in `candy/plugin-cdp` — the core's former minimal CDP client (`browser_cdp.go`, `golang.org/x/net/websocket`) was DELETED when `wl` externalized, so `golang.org/x/net` is now an INDIRECT dependency. |
...
Unmarshal-to-value + re-marshal scrambles comments, key order, and node styles. Every `charly.yml` editor in the authoring surface (`kit.SetByDotPath` in `sdk/kit/yaml.go`; `AddBox` / `AddCandyToBox` / `RemoveCandyFromBox` in `charly/scaffold_project.go`; `appendCandyPackages` in `candy/plugin-candy/command.go`) navigates `*yaml.Node` trees directly and only serializes with `yaml.Marshal(root)` at the very end. Tests (`sdk/kit/yaml_test.go`, `charly/scaffold_project_test.go`) explicitly verify that leading file comments, sibling keys, and per-key inline comments all survive round trips.
