...
- `sdk/spec/*_wire.go` (deploy/init/arbiter/gpu/doctor/enc/feature/k8sgen/resource/settings/substrate_template/agent/distro/vm) — the host↔plugin & render-context wire structs, **CUE-SOURCED per the mandate** (`sdk/schema/*.cue` → generated). `buildctx.cue` (`InstallContext`/`BuildStageContext`) and `clean.cue` (`RetentionRequest`/`RetentionReply`) are LANDED — no hand-written file remains for either; any still-hand-written `*_wire.go` in the list above is a conversion-in-progress, NOT a sanctioned exception — convert it to a CUE def (a plain/discriminated struct always generates, referencing a disjunction/state type by name where one is genuinely needed).
