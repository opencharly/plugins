...
The host-side VM venue lifecycle (boot the domain, build the guest SSH executor, nested pod-in-guest, teardown, the `charly vm` Start/Stop/Status/Logs/Shell/Rebuild) lives in the host-coupled `vmSubstrateLifecycle` methods (`charly/vm_deploy_lifecycle.go`), implementing the `substrateLifecycle` interface (`charly/deploy_substrate_lifecycle.go`). It is EXTERNALIZED (M4b): `candy/plugin-deploy-vm` (`Lifecycle:true`) forwards each lifecycle Op to the hidden `charly __vm-lifecycle` over the "cli" seam — the host-coupled logic stays core, the plugin is the substrate interface (like pod, M4a). Both pod and vm own a real venue lifecycle.
