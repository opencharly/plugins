...
`ReverseOp`, `ReverseOpPluginScript`, `DeployTargetDispatchRequest`, `DeployTargetDispatchReply`) live
in `sdk/spec/deploy_wire.go`, SDK-importable so an out-of-tree deploy plugin constructs the same
structs (R3).
