...
- Round-trip: `LabelShell` (`ai.opencharly.shell`) carries the merged set; `CollectShell` builds it at `charly box build` time, `ExtractMetadata` parses it at deploy time. The label's Deploy section (a charly.yml `shell:` overlay merged by id) is currently DEAD — its former merger `MergeDeployShell` (+ `replaceShellEntryByID`) had zero real callers and was deleted in the 2026-07-22 dead-code-radical-removal batch, so a deploy-scope `shell:` block parses but does not currently overlay onto the label.
