...
| `charly/deploy_target_pod.go` | DELETED (P11c). The former in-core pod overlay target (`PodDeployTarget`) — the overlay Containerfile synthesis + `tagDeployAlias` + the render helpers — MOVED to `candy/plugin-deploy-pod/overlay.go` (the candy renders the overlay in its own code via `deploykit.OCITarget`). The two pure helpers core still needs (`collectOverlayCandies`, `readImageRegistry`) moved into `charly/build_overlay.go`. The pod config-WRITE path (quadlet) is a SEPARATE batch (#86, excluded from P11c) |
