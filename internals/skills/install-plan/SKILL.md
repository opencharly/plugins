...
for Builder, → `execLocalPkgInstall` for LocalPkgInstall; the act-OpStep `resolveProvisionScript` / `renderOpCommand` path for Op), but its pod-overlay BUILD-emit moved off the former in-core overlay walker into `candy/plugin-installstep`
...
`renderLocalPkgImageInstall` `sdk/deploykit WriteCandySteps` calls for the image build; Op via the SAME
