...
**Forbidden-excuse catalog — the difficulty- AND SIZE-based sibling set.** Distinct from the deferral phrasings above, these use difficulty OR size to justify NOT doing (or scope-cutting) the work — equally forbidden (CLAUDE.md "Hard Cutover by Default"): the difficulty framings "it's complicated" / "it's too difficult" / "it's too hard" / "it's too risky" / "it touches too much" / "it would recreate/duplicate X" / "the spike showed it's hard" / "keep the current approach for now", AND the size framings "it's huge" / "it's a multi-cutover sub-program" / "it's a multi-session effort" / "it can't be finished this session" / "I've scoped it as a focused effort for later" / "ready to grind it whenever you want". Difficulty AND size NEVER justify skipping, shrinking, deferring, keeping-as-is, re-scoping, or REVERTING ALREADY-WORKING CODE; they are inputs to finding the HOW (RDD/a spike — a support tool, never an excuse), and a genuine scope-change is STOP-and-ask, never a unilateral cut. **Size decomposes into atomic per-unit cutovers landed NOW, one after another** — decomposition is forward motion (land unit 1, continue), never a deferral device; reverting working code because the rest is large is the worst form of the excuse, and the excuse stays forbidden when dressed as "an honest assessment" / "a scoping finding" / "the spike's deliverable". The same rule governs architecture placement: the kernel/plugin boundary law places by kind-agnostic mechanism, never by difficulty (`/charly-internals:plugin` "The kernel/plugin boundary law").
