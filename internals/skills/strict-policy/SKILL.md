...
**The spike — RDD's named instrument.** A spike is a TIME-BOXED, THROWAWAY investigation (the industry term: [Spike (software development)](https://en.wikipedia.org/wiki/Spike_(software_development))) scoped to ONE named high-risk unknown, run empirically — on a live `disposable: true` bed, or against the real generator / emitted artifact when the unknown lives there (the same permitted instruments listed above). Its single purpose is to discover **HOW** to build what the plan requires; its deliverable is the **discovered mechanism — the HOW** — plus any doc/skill/memory fix the result forces (R1); NEVER shipped code. Spike code, config, and beds are discarded once the question is answered; only the knowledge lands in the plan. **A spike is a support tool for the implementation, NEVER an excuse to not do something.** It decides neither *whether the work is done* (R10 does) nor *whether the work happens at all* (the plan — an immutable CONTRACT — does): a spike that "refutes" the current approach means *find a different HOW*, never *skip / shrink / defer / keep-as-is / retreat*. Complexity a spike surfaces is a reason to find the right HOW, never grounds to reduce scope; if a HOW-finding genuinely changes the contract's scope, the only response is STOP-and-ask the operator (the **forbidden-excuse catalog** in CLAUDE.md "Hard Cutover by Default" binds here). The spike/R10 boundary: a spike proves an ASSUMPTION during planning, R10 proves the FINAL code — a spike pass never authorizes a commit. The SDD variant is the SCHEMA SPIKE: prove a high-risk schema shape — the def compiles, the generated type round-trips, the gate accepts/rejects as intended — BEFORE code is written against it (CLAUDE.md "Schema Driven Design (SDD)"; the worked example is the def-level `@go(CharlyName)` breakage in `/charly-internals:go`). A RELOCATION/FOLD variant is the TRACE SPIKE: for moving or folding existing code, report the MEASURED file-by-file trace (real call-graph, real LOC, real collectibility — never an assumed shape) BEFORE any mass edit; this repeatedly corrected a stale brief premise this session (three separate times), so treat a per-file trace as the recommended spike form whenever a residue/fold brief asserts a shape sight-unseen.
...
**Forbidden-excuse catalog — the difficulty-, SIZE-, and PRIORITY-based sibling set.** Distinct from the deferral phrasings above, these use difficulty, size, or self-assigned "priority" to justify NOT doing (or scope-cutting) the work — equally forbidden (CLAUDE.md "Hard Cutover by Default"): the difficulty framings ("it's complicated / too difficult / too hard / too risky / touches too much / would recreate/duplicate X / the spike showed it's hard / keep the current approach"), the size framings ("it's huge / a multi-cutover sub-program / a multi-session effort / can't be finished this session / scoped as a focused effort for later / ready to grind whenever you want"), the CONTEXT framings ("I've run out of / exhausted context / continuing would leave a broken, unbuildable tree / it needs a fresh session (or fresh context) / the correct next step is a fresh session" — "would leave a broken tree" is a FABRICATED excuse with zero evidence and is never raised; running low on context is answered by compacting-and-continuing or DELEGATING the unit to a fresh teammate / sub-agent, whose whole purpose is to carry it forward with a fresh context budget, never by stopping — CLAUDE.md "Agents, Workflows & Teams"), AND the priority framings ("it's secondary / lower-priority / a nice-to-have / optional / not required / a design refinement / NON-\<gate\>, so not required"). NONE ever justifies skipping, shrinking, deferring, keeping-as-is, re-scoping, or REVERTING ALREADY-WORKING CODE; they are inputs to finding the HOW (RDD/a spike — a support tool, never an excuse), and a genuine scope-change is STOP-and-ask, never a unilateral cut. **A large cutover decomposes into atomic per-unit cutovers landed NOW, one after another** — decomposition is forward motion (land unit 1, continue), never a deferral device, and never a SIZING PREFERENCE (it is the too-BIG remedy; the too-SMALL remedy is BATCHING — see the Cutover Sizing Law, `/charly-internals:cutover-policy` "Cutover sizing — the batch law", whose decision procedure settles which applies); reverting working code because the rest is large is the worst form of the excuse (a spike that PROVED the mechanism means keep-and-land, never revert-and-defer), and the excuse stays forbidden when dressed as "an honest assessment" / "a scoping finding" / "the spike's deliverable" / "exhaustively confirmed scope". The same rule governs architecture placement: the kernel/plugin boundary law places by kind-agnostic mechanism, never by difficulty (`/charly-internals:plugin` "The kernel/plugin boundary law").
