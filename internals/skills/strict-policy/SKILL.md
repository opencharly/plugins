...
| `podman image prune` / `podman rmi $(podman images -f dangling=true -q)` | `charly clean --deep` (`--dry-run` reports the would-remove count + reclaimable bytes; store-wide untagged-image purge, in-use/live-build skipped) |
