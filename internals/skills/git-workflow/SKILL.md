...
the self-approval, clears them. That standing authorization is what this repo records.
Note `gh pr merge --auto` is explicitly carved OUT of Merge Without Review on a
protected repo ("`--auto` queues until reviews+checks pass; the gate is
server-enforced"). The load-bearing discipline is unchanged: **only a FRESH
