...
SUPERPROJECT.** The standing `autoMode.allow` rule is committed in the SUPERPROJECT's
`.claude/settings.json`, so it ships to everyone who clones and the loop is autonomous
by default. But Claude Code resolves `.claude/settings.json` from the AGENT'S PROJECT
ROOT, which is its working directory. A validator told to work *inside* `plugins/` or
