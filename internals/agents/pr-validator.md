...
superproject's `permissions.allow` AND any standing `autoMode.allow` rule the operator has
installed there — and your `success` status POST is then denied as Self-Approval ("the only
authorization comes from a `<teammate-message>`"). Validate a submodule PR from the superproject root using literal
